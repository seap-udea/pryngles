...
class Util(object):
    ...
    def calcAngularDistance(pos1, pos2):
        ...
        dlat = lat2 - lat1
        dlon = lon2 - lon1
        a = mh.sin(dlat / 2)**2 + mh.cos(lat1) * \
            mh.cos(lat2) * mh.sin(dlon / 2) ** 2
        c = 2 * mh.arctan2(a**0.5, (1 - a)**0.5)
        return c

    def limbDarkeningNormalization(cs=[0.6562]):
        def integrand(rho):
            return Util.limbDarkening(rho, 1, cs) * 2 * mh.pi * rho
    ...
    def limbDarkening(rho, Rs=1, cs=[0.6562], N=1):
        ...
        order = len(cs)
        if order == 0:
            nume = np.ones_like(rho)
        elif order == 1:
            nume = 1 - cs[0] * (1 - mu)
        elif order == 2:
            nume = 1 - cs[0] * (1 - mu) - cs[1] * (1 - mu)**2
        elif order == 3:
            nume = 1 - cs[0] * (1 - mu) - cs[1] * (1 - mu**1.5) - cs[2] * (1 - mu**2)
        elif order == 4:
            nume = 1 - cs[0] * (1 - mu**0.5) - cs[1] * (1 - mu) - cs[2] * \
                (1 - mu**1.5) - cs[3] * (1 - mu**2)
        else:
    ...
    def calcRingedPlanetArea(Rp, fi, fe, ir, beta=1):
        ...
        if fi * cosir > 1:
            ...
            ri2 = fi**2 * cosir * 2 / np.pi * \
                np.arcsin(yi) - 2 / np.pi * np.arcsin(yi * fi * cosir)
        ri2 = beta * ri2
        ...
        if fe * cosir > 1:
            ...
            re2 = fe**2 * cosir * 2 / np.pi * \
                np.arcsin(ye) - 2 / np.pi * np.arcsin(ye * fe * cosir)
        re2 = beta * re2
...
class Sample(object):
    ...
    def _calcDistances(self):
        ...
        """
        self.ds = np.array([self._closestDistance(self.ss[i], self.ss)
    ...
    def plotSample(self, **args):
        ...
        if self.dim == 2:
            ...
            self.ax.scatter(self.ss[:, 0], self.ss[:, 1],
                            self.ss[:, 2], **sargs)
            ...
            self._setEqualAspectRatio3D(
                self.ss[:, 0], self.ss[:, 1], self.ss[:, 2])
...
class RingedPlanet(object):
    ...
    def _setEquEclTransform(self):
        ...
        ez_equ = self.nr_ecl
        self.M_equ2ecl = np.array(
            list(np.vstack((ex_equ, ey_equ, ez_equ)).transpose())).reshape((3, 3))
    ...
    def _updateSampling(self, Ns, Np, Nr, Nb):
        ...
        self.rps_ecl = np.array(
            [spy.mxv(self.M_equ2ecl, self.rps_equ[i]) for i in range(self.Np)])
        self.eps_ecl = np.array([Util.transformRecLat(
            self.rps_ecl[i]) for i in range(self.Np)])
        ...
        cond = np.sqrt(rsample.ss[:, 0]**2
                       + rsample.ss[:, 1]**2) >= (self.Ri / self.Re)
        self.Nr = cond.sum()
        ...
        self.rrs_ecl = np.array(
            [spy.mxv(self.M_equ2ecl, self.rrs_equ[i]) for i in range(self.Nrt)])
        self.ers_ecl = np.array([Util.transformRecLat(
            self.rrs_ecl[i]) for i in range(self.Nrt)])
    ...
    def _updateStellarPosition(self, x, kepler=False):
        ...
        if kepler:
            ...
            self.f = 2 * mh.arctan(np.sqrt((1 + self.e)
                                           / (1 - self.e)) * mh.tan(self.E / 2))
            self.lamb = self.lambq + self.f
        else:
            self.lamb = x
            self.f = self.lamb - self.lambq
            self.E = 2 * mh.arctan(np.sqrt((1 - self.e)
                                           / (1 + self.e)) * mh.tan(self.f / 2))
        ...
        r /= self.Rstar
        self.rstar = r
        x = r * mh.cos(self.f) * mh.cos(self.lambq) - r * \
            mh.sin(self.f) * mh.sin(self.lambq)
        y = r * mh.cos(self.f) * mh.sin(self.lambq) + r * \
            mh.sin(self.f) * mh.cos(self.lambq)
        z = 0
    ...
    def _updateSamplingObserver(self):
        ...
        self.rps_obs = np.array(
            [spy.mxv(self.M_ecl2obs, self.rps_ecl[i]) for i in range(self.Np)])
        self.nps_obs = self.rps_obs / \
            np.linalg.norm(self.rps_obs, axis=1)[:, np.newaxis]

        # Ring sampling points coordinates {obs}
        self.rrs_obs = np.array(
            [spy.mxv(self.M_ecl2obs, self.rrs_ecl[i]) for i in range(self.Nrt)])
    ...
    def _updateIllumination(self):
        ...
        self.ip[:] = False
        cond = Util.checkAngle(
            self.eps_ecl[:, 0], self.estar_ecl[0], 90 * DEG) * (~self.sp)
        self.ip[cond] = True
        # Indirect illumination
        self.np[(self.vp) * (~self.sp)
                * ((self.eps_equ[:, 1] * self.estar_equ[1]) > 0)] = True

        # Ring
        if self.behavior["shadows"]:
            self._updateRingsShadow(self.estar_equ, self.sr)
        self.ir[:] = False if (
            self.eobs_equ[1] * self.estar_equ[1]) <= 0 else True
        self.fr = (~self.ir)
        self.ir *= (~self.sr)
    ...
    def _updateTransitability(self):
        ...
        # Planet
        self.rhops = ((self.rps_obs[:, 0] - self.rstar_obs[0])
                      ** 2 + (self.rps_obs[:, 1] - self.rstar_obs[1])**2)**0.5
        sp = (self.rhops <= self.Rs) * (self.rps_obs[:, 2] >= 0)
        self.tp = sp if ~side else self.tp
        self.cp = sp if side else self.cp
        # Ring
        self.rhors = ((self.rrs_obs[:, 0] - self.rstar_obs[0])
                      ** 2 + (self.rrs_obs[:, 1] - self.rstar_obs[1])**2)**0.5
    ...
    def _updatePlanetShadow(self, epos=[0, 0], mask=None):
        ...
        """
        cond = Util.checkAngle(
            self.eps_ecl[:, 0], epos[0], 90 * DEG) * (np.sign(epos[1]) * self.eps_equ[:, 1] <= 0)
        delta = np.abs(self.eps_equ[cond, 1])
        dalpha = np.abs(Util.difAngles(self.eps_equ[cond, 0], epos[0]))
        num = np.sin(delta)
        deni = ((self.Ri / self.Rp)**2 - np.sin(dalpha)**2
                * np.cos(delta)**2)**0.5 - np.cos(dalpha) * np.cos(delta)
        di = np.arctan(num / deni)
        dene = ((self.Re / self.Rp)**2 - np.sin(dalpha)**2
                * np.cos(delta)**2)**0.5 - np.cos(dalpha) * np.cos(delta)
    ...
    def _updateRingsShadow(self, epos=[0, 0], mask=None):
        ...
        cond = Util.checkAngle(self.ers_ecl[:, 0], np.mod(
            epos[0] + np.pi, 2 * np.pi), delta)
        # Difference in right ascension with respect to star
        dalpha = np.abs(self.ers_equ[cond, 0] - epos[0])
        # Critical distance to shadow
        rs = self.Rp / (np.cos(dalpha)**2
                        * mh.sin(np.abs(epos[1]))**2 + np.sin(dalpha)**2)**0.5
        # Shadow condition
    ...
    def plotRingedPlanet(self, view='side',
        ...
        if not showfig:
            plt.ioff()
        label = rf"$t/T={self.t / self.T:.2f}$, $r/a={self.rstar * self.Rstar / self.a:.2f}$, $\lambda={self.estar_ecl[0]*RAD:+0.2f}^\circ$ [ $i={self.i*RAD:g}^\circ$, Obs ($\lambda$,$\\beta$) : ({self.eobs_ecl[0] * RAD:g}$^\circ$,{self.eobs_ecl[1] * RAD:g}$^\circ$) ] "
        ...
        # Plot from ecliptic
        # ========================================
        fig1, axs = plt.subplots(1, figsize=(
            self._plot["fs"], self._plot["fs"]))
        ...
        if axis:
            ...
            ax.axhline(0.0, xmin=0.5, xmax=1, color=fccolor,
                       lw=1, zorder=-100, alpha=0.3)
            ax.axvline(0.0, ymin=0.5, ymax=1, color=fccolor,
                       lw=1, zorder=-100, alpha=0.3)
            ax.text(1.0, 0.5, xaxis, fontsize=10, alpha=0.5,
                    transform=ax.transAxes, ha='right', color=fccolor)
            ax.text(0.5, 1.0, yaxis, fontsize=10, alpha=0.5,
                    transform=ax.transAxes, va='top', color=fccolor)
        ...
        # Plot from observer
        # ========================================
        fig2, axs = plt.subplots(1, figsize=(
            self._plot["fs"], self._plot["fs"]))
        ...
        if axis:
            ...
            ax.axvline(0.0, ymin=0.5, ymax=1, color=fccolor,
                       lw=1, zorder=-100, alpha=0.3)
            ax.text(1.0, 0.5, r"$x_\mathrm{obs}$", fontsize=10, alpha=0.5,
                    transform=ax.transAxes, ha='right', color=fccolor)
            ax.text(0.5, 1.0, r"$y_\mathrm{obs}$", fontsize=10, alpha=0.5,
                    transform=ax.transAxes, va='top', color=fccolor)
        ...
        # Plot from star
        # ========================================
        fig3, axs = plt.subplots(1, figsize=(
            self._plot["fs"], self._plot["fs"]))
        fig3.patch.set_facecolor(bgcolor)
        ax = axs

        # Rotate to stellar reference system
        M_equ2str = spy.mxm(spy.rotate(
            self.estar_ecl[0], 3), spy.rotate(self.i, 1))
        rps_str = np.array([spy.mxv(M_equ2str, self.rps_equ[i])
                            for i in range(self.Np)])
        rrs_str = np.array([spy.mxv(M_equ2str, self.rrs_equ[i])
                            for i in range(self.Nrt)])
        ...
        if axis:
            ...
            ax.axvline(0.0, ymin=0.5, ymax=1, color=fccolor,
                       lw=1, zorder=-100, alpha=0.3)
            ax.text(1.0, 0.5, r"$y_\mathrm{str}$", fontsize=10, alpha=0.5,
                    transform=ax.transAxes, ha='right', color=fccolor)
            ax.text(0.5, 1.0, r"$z_\mathrm{str}$", fontsize=10, alpha=0.5,
                    transform=ax.transAxes, va='top', color=fccolor)
        ax.axis("off")
    ...
    def animateRingedPlanet(self, nframes=1,
        ...
        for n, x in iterator:
            ...
            if verbose:
                print(f"Creating image {n+1}/{nframes}: {label}...")
    ...
    def _calcReflectionCoefficient(self, eta, zeta, gamma0=1):
        ...
        """
        rho0 = gamma0 * \
            self.fint(gamma0, eta)[0] * self.fint(gamma0, zeta)[0] / (4 * (eta + zeta))
        return rho0
    ...
    def _findGamma(self):
        ...
        def function(gamma0):
            return self._calcSphericalAlbedo(gamma0) - self.AS
    ...
    def _calcLambertianAlbedoPlanet(self, eta):
        ...
        """
        def integrand(zeta):
            return self._calcReflectionCoefficient(eta, zeta, gamma0=self.gamma0) * zeta
    ...
    def _calcLambertianAlbedoRing(self, eta, gammap0=1, reflection_law=None):
        ...
        def integrand(zeta):
            return reflection_law(eta, zeta) / eta
    ...
    def _accelerateLambertianAlbedoRing(self):
        ...
        ALs = np.array([self._calcLambertianAlbedoRing(eta,
                                                       ...
                                                       reflection_law=self.reflection_rings_law)
                        for eta in etas])
        self.getLambertianAlbedoRing = interp1d(etas, ALs)

    def _findGammap(self, reflection_law=None):
        def function(gammap0):
            return self._calcLambertianAlbedoRing(1, gammap0, self.reflection_rings_law) - self.AL
    ...
    def _updateGeometricalFactors(self):
        ...
        # Scattered angles
        self.etars = np.abs(
            np.inner(self.nstar_equ, self.nr_equ)) * np.ones(self.Nrt)
        # is "sin" because of definition of io
        self.zetars = np.abs(self.cosio) * np.ones(self.Nrt)
    ...
    def _updateIncomingStellarFlux(self):
        ...
        cond = (self.ip)
        self.fluxips[cond] = (self.normp * self.afp
                              / (4 * mh.pi * self.rstar**2)) * self.etaps[cond]
        # Ring
        self.fluxirs = np.zeros_like(self.ar, dtype=float)
        cond = (self.ar) * (np.arange(self.Nrt) < self.Nr)
        self.fluxirs[cond] = (self.normr * self.afr
                              / (4 * mh.pi * self.rstar**2)) * self.etars[cond]
    ...
    def updateTransit(self):
        ...
        self.Tip[cond] = Util.limbDarkening(
            self.rhops[cond], 1, self.limb_cs, self.normlimb) * self.afps[cond]
        ...
        self.Tir[cond] = self.betas * Util.limbDarkening(
            self.rhors[cond], 1, self.limb_cs, self.normlimb) * self.afrs[cond]

    def _getFacetsOnSky(self, r_equ, observing_body="planet"):
        """
        Given an *observing* facet in body1 (planet or ring), get *observable*
        facets of the body2 (ring or planet) which are visible in the sky
        ...
        if observing_body == "ring":
            ...
            zetaijs = np.einsum(
                'ij, ij->i', self.rps_equ[cond], rijs_equ) / (self.Rp * rijs)
    ...
    def updateShine(self):
        ...
        for i in activep:
            msp, rijs, etaijs, zetaijs = self._getFacetsOnSky(
                self.rps_equ[i], observing_body="planet")
            self.Sip[i] = ((self.fluxirs[msp] * zetaijs) * (self.normp
                                                            * self.afp / (4 * mh.pi * rijs**2))
                           * etaijs * self.zetaps[i]).sum()
            # self.Sip[i]=((self.fluxirs[msp])*(self.normp*self.afp/(4*mh.pi*rijs**2))*etaijs).sum()
        # Ring
        self.Sir = np.zeros(self.Nr)
        cond = (self.ar[self.irn]) * (self.nr[self.irn]) * \
            (~self.tp[self.irn]) * (~self.cp[self.irn])
        activer = np.arange(self.Nr)[cond]
        for i in activer:
            msr, rijs, etaijs, zetaijs = self._getFacetsOnSky(
                self.rrs_equ[i], observing_body="ring")
            self.Sir[i] = ((self.fluxips[msr] * zetaijs) * (self.normr * self.afr / (4 * mh.pi * rijs**2))
                           * etaijs * self.zetars[i]).sum()
...
class Extra(object):
    def drawPryngles(iobs=1, dark=False):
        ...
        title = rf"$a={P.a:g}$ au, $i={P.i * RAD:g}^\circ$ ($i_0={P.io * RAD:.1f}^\circ$)" + \
                rf", $\lambda_\mathrm{{q}}={P.lambq * RAD:g}^\circ$, Obs ($\lambda$,$\\beta$)" + \
                rf" : ({P.eobs_ecl[0]*RAD:g}$^\circ$,{P.eobs_ecl[1] * RAD:g}$^\circ$)"
        theta = np.linspace(0, 2 * np.pi, no) * title
        x = np.cos(theta)
        y = np.sin(theta)
        ...
        p = ax.scatter(x, y, (z2), marker=".", c=z1 + z2, s=7,
                       cmap=cmap, alpha=alpha, edgecolors=None)
        cb = plt.colorbar(p, orientation="horizontal", fraction=0.03, pad=-0.2)
        cb.set_label(r"Pryngles", color=fcolor,
                     fontsize=40, fontname="Special Elite")
        ...
        for i in np.linspace(0, 1, 20):
            ax.scatter(0, 0, 0, marker=".", s=1000 * 5 * i,
                       color="gold", alpha=1 - 0.9 * i)
