...
class System(PrynglesCommon):
    ...
    def add(self,kind="Star",parent=None,**props):
        ...
        exec(f"self.__body={kind}(parent=parent,**props)")
        ...
        #Set the root body in the system
        if self.__body.parent is None:
            if self.root:
                raise AssertionError("A root body has been already set in {self.root}")
