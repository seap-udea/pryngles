...
##################################################################
# Pryngles release script (adapted from contrib/pymcel)
#
# - Updates the version in:
#   - setup.py
#   - src/pryngles/version.py
...
VERSION_PY="$ROOT_DIR/src/pryngles/version.py"
...
Examples:
  bash bin/release.sh test 1.0.0
...
files = {
    ...
    "version.py": root / "src/pryngles/version.py",
}
...
if ! printf '%s' "$VERSION_NEW" | grep -Eq '^[0-9]+(\.[0-9]+)+([a-zA-Z0-9\.\-]+)?$'; then
  die "Invalid version '$VERSION_NEW'. Use something like 1.0.0 or 1.0.0.1."
fi
...
log "Releasing Pryngles $VERSION_NEW (current: $CURRENT_SETUP) in '$TYPE' mode..."

BACKUP_DIR="$(mktemp -d -t pryngles-release.XXXXXX)"
