...
def matrix(M):
    x = len(M)
    y = len(M[0])
    arr = (ctypes.POINTER(ctypes.c_double) * x)()  # allocate array of pointers in X dimension
    for i in range(x):
        arr[i] = (ctypes.c_double * y)(*M[i]) # allocate array in Y dimension and populate it
    return arr
...
suma=cpixx.sum_matrix(matrix(M),vector(sumas),3,3);
print(suma)
print(sumas)
exit(0);
